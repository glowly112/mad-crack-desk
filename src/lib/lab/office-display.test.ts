...
test("measuring row shows today's paper settles — measuring is not KEEP", () => {
  ...
  assert.equal(rows[0]?.cumW, "0");
  assert.equal(rows[0]?.cumL, "1");
  assert.equal(rows[0]?.cumN, "1");
  assert.equal(rows[0]?.paperUnit, "−1.50u/n");
  assert.equal(rows[0]?.todayW, "0");
  assert.equal(rows[0]?.todayL, "1");
  assert.equal(rows[0]?.todayU, "−1.50u");
...
test("office paper survives day roll — prior-day settles stay cumulative", () => {
  ...
  assert.equal(nzRow?.cumW, "1");
  assert.equal(nzRow?.cumL, "0");
  assert.equal(nzRow?.cumN, "1");
  assert.equal(nzRow?.todayW, EMPTY);
  assert.equal(nzRow?.todayL, EMPTY);
  assert.equal(nzRow?.todayU, EMPTY);
  assert.equal(gbRow?.paperPnl, "−1.00u");
  assert.equal(gbRow?.paperCounts, "since armed");
  assert.equal(gbRow?.cumW, "0");
  assert.equal(gbRow?.cumL, "1");
  assert.equal(gbRow?.cumN, "1");
  assert.equal(gbRow?.todayW, "0");
  assert.equal(gbRow?.todayL, "1");
  assert.equal(gbRow?.todayU, "−1.00u");
